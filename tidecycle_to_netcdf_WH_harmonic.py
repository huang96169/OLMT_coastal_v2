...
sal_data = pandas.read_csv('/home/whf/E3SM/OLMT_coastal_v2/Annapolis_sal_elev_datenum_MSL.csv')
...
time=xarray.Variable(dims='time',data=time[:],attrs={'units':'hours'})
...
#print(time[0])
tide_height=xarray.Variable(dims=('time','gridcell'),data=tide2(arange(len(time),dtype=float)*3600,amp=tide_coeffs['Amplitude'].values,phase=tide_coeffs['Phase'].values,speed=tide_coeffs['Speed'].values,datum=0.0)[:,None],attrs={'units':'m'})
...
d.to_netcdf('./Annapolis_harmonic_sal_35yrs_new.nc')
