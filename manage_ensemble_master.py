...
for s in pfile:
  ...
  nparms = nparms+1
pfile.close()
parm_row = np.zeros([nparms], np.float64)-999
if (rank == 0):
  parms = np.zeros([nparms, options.n], np.float64)-999
  sse_ensemble = np.zeros([options.n], np.float64)-999      
